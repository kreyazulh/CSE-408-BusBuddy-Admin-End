...
onMount(async () => {
    try {
      const response = await fetch('/api/trip/liveTrips');
      if (!response.ok) {
...
<main class="flex min-h-screen min-w-full">
  <div>
   ...
   <div class="flex-1 ml-56 w-full px-4 py-8 bg-white-700">
    ...
    <!-- Search Bar and Remind Button-->
<div class="flex mb-4 justify-end items-center">
<button on:click={() => navigate('/livetrips/remind')} class="bg-maroon-500 text-white-700 font-semibold py-2 px-4 border border-maroon-700 rounded shadow mr-2">
  Remind
</button>
