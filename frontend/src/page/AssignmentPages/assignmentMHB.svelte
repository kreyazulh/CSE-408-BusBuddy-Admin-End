<script>
    ...
    function handleClick(name) {
     ...
     function showStatus() {
      handleClick("showAll");
      if (showAll) {
        searchRows = assignments.filter((row) => row.status === "Active");
        showAll = false;
      } else {
        searchRows = assignments;
        showAll = true;
      }
    ...
    if (rowIndex !== -1) {
      try {
        ...
        if (response.ok) {
          ...
          rowIndex = assignments.findIndex((r) => r.id === rowDelID);
          if (rowIndex !== -1) {
            assignments.splice(rowIndex, 1);
    ...
    async function getAssgns() {
      ...
      for (let row in assignments){
        for (let key in assignments[row]){
          if (assignments[row][key] === null){
            assignments[row][key] = "N/A";
          }
        }
      }
      console.log(assignments);
    ...
    <div class="flex-1 ml-56 w-full px-4 py-8 bg-white-700">
      ...
      <div class="flex mb-4 justify-end">
        ...
        <button class=" mx-3 bg-indigo-700 hover:bg-indigo-900 text-white-700 font-semibold py-2 px-4 rounded-full focus:translate-y-1.5"
          on:click={gotoAddAssgn}> Add New Assignment
        </button>
        <SearchBar originalRows={assignments} on:searchChanged={handleSearch} />
      </div>
          ...
          <thead>
            <tr>
              <th class="bg-white-700  w-auto">
                ...
                <th class=" bg-white-700  w-auto">
                  <div class="flex items-center justify-center space-x-1 bg-red-70 rounded-full">
                    <span class="pl-2 font-semibold uppercase text-xs text-black-700">Bus</span>
                    <SortingButtons
                      searchRows={searchRows}
                      sortColumn="bus"
                      on:sort={(e) => {
                ...
                <th class="bg-white-700  w-auto">
                  <div class="flex items-center justify-center space-x-1 bg-red-70 rounded-full">
                    <span class="pl-2 font-semibold uppercase text-xs text-black-700">Shift</span>
                    <SortingButtons
                      searchRows={searchRows}
                      sortColumn="shift"
                      on:sort={(e) => {
            ...
            {#each searchRows as row, index}
              {#if index >= (currentPage - 1) * Number(entriesPerPage) && index < currentPage * Number(entriesPerPage)}
                <tr class="{index % 2 === 0 ? 'bg-gray-200' : 'bg-white-700'} hover:bg-gray-300">
                  ...
                  <td class="py-2 pl-2 pr-2 text-center w-auto">{row.shift}</td>
