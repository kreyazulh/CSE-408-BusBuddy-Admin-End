<script>
    ...
    function addNewRow() {
  const newRow = {
    id: '', // You might want to generate a temporary ID or leave it empty until saved
    currentRoute: '',
    busNumber: '',
    driverName: '',
    staffName: '',
  ...
  rows = [newRow, ...rows]; // Add the new row at the beginning of the array
  changedRows.push(newRow.id); // Add the new row's ID to changedRows to enable saving
}
